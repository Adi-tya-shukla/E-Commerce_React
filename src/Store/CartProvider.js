...
const CartProvider = (props) => {
  ...
  useEffect(() => {
    ...
    const fetchCartData = async () => {
      if (emailAdd) {
        try {
          const response = await axios.get(`https://crudcrud.com/api/a4b42bed173e42168f27cbe47bc66d9e/cart`);
 ...
 const   removeFromCartHandler = async (item) => {
    try {
     const id = item.id;
     const response = await axios.delete(`https://crudcrud.com/api/a4b42bed173e42168f27cbe47bc66d9e/cart/${id}`)// deleted from database with id
...
const logInHandler = (token, mail) => {
  ...
  async function userDetailHandler(user) {
    console.log(user);
    const response = await fetch('https://react-proj-f7662-default-rtdb.firebaseio.com/intractedUser.json', {
      method: 'POST',
